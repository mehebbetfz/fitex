...
const EditBodyProfileCtx = createContext<Ctx | null>(null)

export function EditBodyProfileProvider({ children }: { children: React.ReactNode }) {
	const { t } = useLanguage()
	...
	const value = useMemo(
		() => ({ state, setState, loading, submitSave }),
		[state, loading, submitSave],
	)

	return <EditBodyProfileCtx.Provider value={value}>{children}</EditBodyProfileCtx.Provider>
}

export function useEditBodyProfile() {
	const c = useContext(EditBodyProfileCtx)
	if (!c) throw new Error('useEditBodyProfile must be used under EditBodyProfileProvider')
	return c
}
